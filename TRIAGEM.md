...
| **Pendências em aberto** | Os títulos das perguntas de `REVISAO-PENDENTE.md`, na raiz: o que foi examinado e deliberadamente não respondido | Nada, em regra — são repetidas para não se perderem. O arquivo é autossuficiente e pode ser enviado a quem for responder, sem contexto do repositório. Resolver uma é aplicar a mudança e REMOVER a seção, citando a versão no changelog |
