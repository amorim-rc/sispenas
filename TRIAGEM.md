...
| **Pendências em aberto** | Os títulos das perguntas de `REVISAO-PENDENTE.md`, cada um com a marca de submissibilidade — ✅ manda hoje, ⚠️ depende de informação recente, ⛔ ainda pede varredura | Para responder uma: copie o bloco **Instruções ao revisor** mais **uma** pergunta e cole numa conversa nova. Cada pergunta traz o **Formato da resposta**, que é o que permite aplicar sem retrabalho. Resolver = aplicar e REMOVER a seção, citando a versão no changelog |
